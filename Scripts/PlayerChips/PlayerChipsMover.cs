...
{
  private GameField GameField;                        // Скрипт игрового поля
  ...
  public void MoveChip(int playerId, int steps)
  {
    ...
    GetComponent<PlayerChipsAnimator>().AnimateChipMovement(_playersChips[playerId], GameField.GetArrayOfCoord(movementCells));   // Запускаем анимацию движения фишки
  ...
  private void TryApplyTransition(int playerId)
  {
    ...
    int resultCellId = GameField.GetTransitionResultCellId(_playersChipsCellsIds[playerId]);
